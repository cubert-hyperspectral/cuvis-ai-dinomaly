...
This variant intentionally removes the MinMaxNormalizer stage.
Everything else mirrors the original frozen-band training script.
"""
...
@hydra.main(
    config_path="../configs",
    config_name="trainrun/dinomaly_multifile_rgb_frozen_adaclip_bands_no_minmax",
    version_base=None,
)
def main(cfg: DictConfig) -> None:
    logger.info("=== Dinomaly + MultiFile (RGB from frozen AdaCLIP concrete bands, no minmax) ===")
    ...
    dcfg = cfg.dinomaly
    pipeline = CuvisPipeline("dinomaly_multifile_rgb_frozen_adaclip_bands_no_minmax")
    ...
    pipeline.connect(
        (data_node.outputs.cube, selector.cube),
    ...
    pipeline.save_to_file(
        ...
        metadata=PipelineMetadata(
            ...
            description=(
                ...
                f"(nm={target_wl}), no MinMaxNormalizer."
            ),
            tags=["dinomaly", "anomalib", "multifile", "rgb", "custom_wavelengths", "no_minmax"],
            author="cuvis.ai",
        ),
    )
