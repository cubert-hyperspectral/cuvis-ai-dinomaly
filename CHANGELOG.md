...
- **Repointed the lentils tutorial notebooks to `convert_universe`.** The four `notebooks/lentils_anomaly/` tutorials (rgb, cir, adaclip_bands, inference) now download the dataset and call `cuvis_ai_dataloader.data.npz_converter.convert_universe(universe.csv, splits_json=splits/dinomaly.json)` instead of `convert_split_manifest(splits_dinomaly.csv)`, because the HF dataset now ships a `universe.csv` + `splits/*.json` selectors rather than the rich cu3s split CSV. The `lentils_adaclip_bands` cell that derives the AdaCLIP wavelengths reads `materialized_path`; REPRODUCTION.md / LENTILS_TRAINING.md / `utils.py` describe the `convert_universe` flow. Needs the cuvis-ai-dataloader carrying `convert_universe` (>=0.5.0).
