...
- **Bool targets and `validate_args=False` remove the per-step int64 promotion and the sort.** The pixel and image accumulators take the mask as `bool` (torchmetrics' binned update handles it directly) rather than promoting every pixel to int64, and both `BinaryAUROC` objects are constructed with `validate_args=False`, which drops the per-update `unique()` over every pixel. Measured about 28 MiB transient per validation step at 1000x1080 before the change. Floors `cuvis-ai-core>=0.16.3` for the shared `cuvis_ai_core.node.metric_utils` helpers (`subsample_hw`, `warn_below_vectorized_cutoff`); the node warns once per run when the stride pushes the element count to or below torchmetrics' 50 000-element cutoff, where it switches to the vectorized update that allocates an `[N, thresholds]` int64 tensor.
